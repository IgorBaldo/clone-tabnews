...
describe("Use case: Registration Flow (all successful)", () => {
  ...
  test("Receive activation email", async () => {
    ...
    expect(lastEmail.sender).toBe("<contato@talibtech.com.br>");
    expect(lastEmail.recipients[0]).toBe("<registration.flow@curso.dev>");
    expect(lastEmail.subject).toBe("Ative seu cadastro no TalibTech!");
  ...
  test("Get user information", async () => {
    ...
    const userReponseBody = await userResponse.json();

    expect(userReponseBody.id).toBe(createUserResponseBody.id);
  });
});
